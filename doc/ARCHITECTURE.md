...
The counterpart is `SbeeEngine.discardActiveSession()`, for a user who doesn't want to resume the recovered session at all. It calls the new `SessionRepository.deleteSession(id)` on the current `getActiveIncompleteSession()` result. Without a real delete path, a host app's only options were to keep resuming the same abandoned session forever or to hide the resume prompt locally while the underlying row stayed in the database indefinitely.
...
- **Reused Magnitude, Not a New One**: the halving factor is the same one `calculateExerciseSetsCount` already applies for a scheduled deload (its `isDeload` branch) — an already-vetted reduction, not a newly-invented percentage for a similar case. (`PeriodizationScheduler.applyDeload` is a separate, session-level truncation exercised only by its own tests, not the mechanism either reduction here actually uses.) If a deload week and a beginner account coincide, the two compose (quartering, still floored at 1), since both are independently valid reasons to train lighter.
