...
void main() {
  group('PeriodizationScheduler Tests', () {
    test('Next day-type rotates correctly', () {
      ...
      expect(
          PeriodizationScheduler.getNextDayType([]), equals(DayType.moderate));
      ...
      expect(PeriodizationScheduler.getNextDayType([s1]),
          equals(DayType.veryHeavy));
      ...
      expect(PeriodizationScheduler.getNextDayType([s1, s2]),
          equals(DayType.power));

      final s3 = WorkoutSession(
          id: '3', startTime: now, isCompleted: true, dayType: DayType.power);
      expect(PeriodizationScheduler.getNextDayType([s1, s2, s3]),
          equals(DayType.veryLight));

      final s4 = WorkoutSession(
          id: '4',
          startTime: now,
          isCompleted: true,
          dayType: DayType.veryLight);
      expect(PeriodizationScheduler.getNextDayType([s1, s2, s3, s4]),
          equals(DayType.highLactic));

      final s5 = WorkoutSession(
          id: '5',
          startTime: now,
          isCompleted: true,
          dayType: DayType.highLactic);
      expect(PeriodizationScheduler.getNextDayType([s1, s2, s3, s4, s5]),
          equals(DayType.moderate));
    });
    ...
    test('Deload week is determined active on week 5 (index 4)', () {
      ...
      expect(
          PeriodizationScheduler.isDeloadActive(
              completedSessions: history, currentTime: base),
          isFalse);
      expect(
          PeriodizationScheduler.isDeloadActive(
      ...
      expect(
          PeriodizationScheduler.isDeloadActive(
              completedSessions: history,
              currentTime: base.add(const Duration(days: 27))),
          isFalse);
      ...
      expect(
          PeriodizationScheduler.isDeloadActive(
              completedSessions: history,
              currentTime: base.add(const Duration(days: 28))),
          isTrue);
      expect(
          PeriodizationScheduler.isDeloadActive(
              completedSessions: history,
              currentTime: base.add(const Duration(days: 34))),
          isTrue);
      ...
      expect(
          PeriodizationScheduler.isDeloadActive(
              completedSessions: history,
              currentTime: base.add(const Duration(days: 35))),
          isFalse);
    });
    ...
    test('Apply deload cuts volume in half and limits target RPE to 6', () {
      ...
      final sessionActive = PeriodizationScheduler.applyDeload(
          session: originalSession, isDeload: false);
      ...
      final sessionDeload = PeriodizationScheduler.applyDeload(
          session: originalSession, isDeload: true);
